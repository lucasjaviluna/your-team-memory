...
async function getAuthToken(
  ...
  if (!token) {
    ...
    console.error(
      `  ${c.cyan}TEAM_MEMORY_TOKEN${c.reset}=sk-writer-abc123 memory-tui\n`,
    );
    process.exit(1);
  }
  ...
  try {
    ...
    console.log(
      `  ${c.green}✓${c.reset} Usuario    ${c.bold}${data.user.username}${c.reset}  ${c.gray}(${data.user.role})${c.reset}`,
    );
...
async function main() {
  ...
  if (!serverUrl) {
    ...
    console.error(
      `  ${c.cyan}TEAM_MEMORY_URL${c.reset}=http://IP:3100/mcp memory-tui\n`,
    );
    process.exit(1);
  }
  console.log(
  ...
  const healthy = await checkHealth(serverUrl.value);
  process.stdout.write("\r");

  if (!healthy) {
    console.error(
      `  ${c.red}✗${c.reset} El servidor no responde en ${c.bold}${serverUrl.value}${c.reset}\n`,
    );
    process.exit(1);
  }
  console.log(
    `  ${c.green}✓${c.reset} Conectado  ${c.gray}${serverUrl.value.replace(/\/mcp\/?$/, "/health")}${c.reset}`,
  );

  // ── Auth ──────────────────────────────────────────────────────────────────
  const { token: apiToken, isAdmin } = await getAuthToken(
    serverUrl.value,
    explicitToken,
  );
