...
SUMMARY and TASK_CONTEXT are excluded from the duplicate check — they are accumulative by nature.`,
  ...
  server.registerTool(
    ...
    {
      description:
        "Update an existing memory entry: replace or append content, change tags, or change status. Use this instead of save_memory when correcting, extending, or deprecating an entry that already exists. Cannot be used on archived entries.",
  ...
  server.registerTool(
    ...
    {
      description:
        "Search team knowledge using hybrid semantic + keyword search.",
  ...
  server.registerTool(
    ...
    {
      description:
        "Load the full knowledge context for a project at the start of a session.",
...
for future authenticated user tracking — currently ignored.`,
  ...
  server.registerTool(
    ...
    {
      description:
        "Compact old low-access entries into SUMMARYs. Always dry_run first.",
  ...
  server.registerTool(
    "delete_memory",
    {
      description: `Permanently delete a memory entry and its access log.
ADMIN ONLY — requires role: admin.
This action is irreversible. The entry will be gone permanently.
          ...
          content: [
            {
              ...
              text: JSON.stringify({
                success: true,
                message: `Permanently deleted: "${result.title}" [${result.type}]`,
                ...result,
              }),
            },
          ],
...
async function startStreamableHTTP() {
  ...
  // ── Health check — incluye estado de auth ──────────────────────────────────
  ...
  // ── Rutas de autenticación — no requieren auth (son el punto de entrada) ───
  app.use("/auth", authRouter);

  // ── Endpoint MCP — protegido por auth si está habilitada ───────────────────
    ...
    if (authEnabled && req.auth) {
      ...
      if (toolName) {
        ...
        if (!allowed) {
          res.status(403).json({
            jsonrpc: "2.0",
            error: { code: -32603, message: reason ?? "Forbidden" },
            id: null,
          });
          return;
        }
  ...
  app.get("/mcp", (_req, res) => {
    res
      .status(405)
      .json({
        error: "Method Not Allowed — this server runs in stateless mode",
      });
  });
  ...
  app.listen(PORT, "0.0.0.0", () => {
    console.error(
      `[team-memory] 🚀 Streamable HTTP server  →  http://0.0.0.0:${PORT}`,
    );
    console.error(`[team-memory]    /mcp      — endpoint MCP (POST)`);
    console.error(`[team-memory]    /health   — estado del servidor`);
    console.error(`[team-memory]    /auth/*   — endpoints de autenticación`);
    console.error(
      `[team-memory]    Auth:     ${authEnabled ? "✅ habilitada" : "⚠️  deshabilitada (AUTH_ENABLED=false)"}`,
    );
  });
}
...
async function main() {
  ...
  console.error(
    `[team-memory] ${ollamaOk ? "✅" : "⚠️ "} Ollama ${ollamaOk ? "conectado" : "no disponible"}`,
  );
  console.error(
    `[team-memory] ${chatOk ? "✅" : "⚠️ "} Chat model ${chatOk ? "disponible" : "no encontrado — compact_memory fallará"}`,
  );
...
main().catch((err) => {
  console.error("[team-memory] Fatal:", err);
  process.exit(1);
});
