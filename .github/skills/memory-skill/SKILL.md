...
> **Authoritative reference**: The complete trigger table, self-evaluation checklist, deduplication rules, writing format, and output contracts are defined in `.github/agents/memory.agent.md`. This file summarizes the key operational points and adds context for reading and writing each memory category.
...
Full detail in `memory.agent.md` (Area Resolution section).
...
Use the Memory agent output contract from `memory.agent.md` (Mode 1: Bootstrap). Keep it concise but sufficient.
...
See the full trigger table in `memory.agent.md`. Summary (conditions unchanged from the legacy system; only the action target changed):
...
- For `TASK_CONTEXT` (T6) entries: use the `**Gap** / **Suggested fix** / **Evidence**` format defined in `memory.agent.md`
- Never call `update_memory` on an `archived` entry — fall back to `save_memory` instead

### Step 4: Report consolidation

Return using the output contract from `memory.agent.md` (Mode 2: Consolidation):
