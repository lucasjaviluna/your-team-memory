 ...
 *
 * Uso:
 *   node install.mjs                                       (usa defaultUrl de team-memory.config.json)
 *   node install.mjs --url http://IP-SERVIDOR:3100/mcp     (override explícito)
 *   node install.mjs --url http://IP-SERVIDOR:3100/mcp --dry-run
 *   node install.mjs --url http://IP-SERVIDOR:3100/mcp --yes
...
const ONLY       = getArg('--only') // ej: claude,vscode,copilot-cli,cursor

// ── Resolución de URL: --url explícito > team-memory.config.json > error ────
...
async function main() {
  ...
  if (!SERVER_URL) {
    err('Falta --url y no hay team-memory.config.json con defaultUrl.')
    info('Uso: npx github:tu-org/team-memory install --url http://IP-SERVIDOR:3100/mcp')
    process.exit(1)
  }
